...
Medium<Float, Spectrum>::sample_interaction_real(const Ray3f &ray,
    ...
    while (loop(active))
    {
        ...
        if (has_supergrid)
        {
            ...
            Mask reached_dda = active_dda && (local_majorant > 0) && (t_precise < maxt) && (tau_next >= desired_tau);
        ...
        dr::masked(mei.sigma_s, active_medium) = local_sigma_s;
        dr::masked(mei.sigma_n, active_medium) = local_sigma_n;
        dr::masked(mei.sigma_t, active_medium) = local_sigma_t;
    ...
    mei.combined_extinction = dr::detach(get_majorant(mei, mei.is_valid()));
...
MI_VARIANT
std::pair<Spectrum, PCG32<typename Medium<Float, Spectrum>::UInt32>>
Medium<Float, Spectrum>::estimate_transmittance(const Ray3f &ray, PCG32<UInt32> sampler, UInt32 channel,
                            Mask _active) const {
